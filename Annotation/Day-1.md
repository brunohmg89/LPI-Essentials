...
- `cd -` colocando um `-` ele entra no último diretório acessado
...
- `ls -lha` comando `ls` serve para listar arquivos e diretórios.
- `ls -lha` comando `-a` serve para lista todos(all) arquivos, inclusive os arquivos ocultos que iniciam com um `.` no inicio do arquivo
- `ls -lha` comando `-l` serve para trazer mais informações sobre os arquivos.
- `ls -lha` comando `-h` serve para "humanizar" as informações, para deixar mais clara de leitura.
- `clear` ou `CTRL + l` serve para "limpar" a tela
- `history` serve para trazer um histórico de comandos utilizados
- Diretórios `.` e `..`. O `.` simboliza o diretório atual e o `..` simboliza o diretório anterior.
- `pwd` mostra qual o diretório atual que estamos.
